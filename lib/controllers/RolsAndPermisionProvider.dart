...
class RolesAndPermProvider with ChangeNotifier {
  ...
  Future<void> getAllPerm(BuildContext context) async {
    var resp = await _roleApi.getAllRoleAndPerm(
        businessId: Controllers.businessProvider(context).currentBusiness?.id??""
    );
            ...
            children: [
                                  ...
                                  for(String key in i.keys)
                                    DataRow(
                                      ...
                                      DataCell(Text(key)),
                                          ...
                                          //
                                      DataCell(Checkbox(
                                        value: _selectedPermissionData[key]?.contains('read')??false,
                                        onChanged: (c){
                                          ...
                                          //
                                      DataCell(Checkbox(
                                        value: _selectedPermissionData[key]?.contains('write')??false,
                                        onChanged: (c){
              ...
              ElevatedButton(onPressed: ()async{
                ...
                if(res.statusCode==201)
                  {
                    getAllPerm(context);
                    MyHelper.mySnakebar(context, "${res.data["message"]}",color: Colors.green);
                    Navigator.pop(context);
                  }
                else
                  {
                    MyHelper.mySnakebar(context, "${res.statusCode}\n${res.data}");
                  }
