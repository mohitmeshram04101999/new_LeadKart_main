...
class _EditBusinessScreenState extends State<EditBusinessScreen> {
  ...
  void initState() {
    super.initState();
    Controllers.editBusinessProvider(context,listen: false).init(context);
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: ()async {
        Controllers.editBusinessProvider(context,listen: false).clear();
        return true;
      },
      child: Scaffold(
        ...
        appBar:  AppBar(
          ...
          title: Text('Edit Yor Business Details',style: TextStyle(fontSize: SC.from_height(21)),),
        ),
        body: Consumer<EditBusinessProvider>(
          builder: (a,p,c){
            return ListView(
                              ...
                              child: Wrap(
                                children: [
                                  for(var chip in p.service_ids)
                                    ...
                                    onDeleted: (){
                                      Controllers.editBusinessProvider(context,listen: false).removeServiceId(chip.id!);
                                    },),
                                          ...
                                          return [
                                            for(var i in p3.allSrvices)
                                              if(p.service_ids.every((elemt)=>elemt.id!=i.id))
                                                PopupMenuItem(
                                                    onTap: (){
                                                      Controllers.editBusinessProvider(context,listen: false).addService(i);
                                                    },
